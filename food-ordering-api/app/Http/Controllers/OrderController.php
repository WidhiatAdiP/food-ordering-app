...
class OrderController extends Controller
{
    ...
    public function setAsDone($id)
    {
        ...
        if($order->status != 'ordered') {
            return response('Order cannot set to done bcz the status is not ordered.', '403');
        }
    ...
    public function payment($id)
    {
        ...
        if($order->status != 'done') {
            return response('Payment cannot be done bcz the status is not done.', '403');
        }
    ...
    public function orderReport(Request $request)
    {
        ...
        $orders = $data->select('id', 'customer_name', 'table_no', 'order_date', 'order_time', 'status', 'total', 'waitress_id','cashier_id')
        ->with(['waitress:id,name','cashier:id,name'])
        ->get();
